...
# def to_sparse_tensor(numpy_H):
#     """
#     将稠密矩阵转换为稀疏张量。
#     关键修改：所有操作必须在 CPU 上完成，绝对不要在这里 .to(device)
#     """
#    # 1. 直接转 tensor（和你原来代码一样）
#     H_tensor = torch.from_numpy(numpy_H)
    
#     # 2. 直接转稀疏 COO（和你原来代码一样）
#     sparse_tensor = H_tensor.to_sparse_coo().coalesce()  # 加 coalesce 避免报错
#     return sparse_tensor

def convert_H_to_HyperGT_adjs(H_sparse):    
    row, col = H_sparse.indices()
    num_nodes = H_sparse.size(0)
    num_hyperedges = H_sparse.size(1)
    hypernode_idx = num_nodes + col
    edge_index_bipart = torch.stack([row, hypernode_idx])
    adjs = []
    adj_bipart, _ = remove_self_loops(edge_index_bipart)
    total_nodes = num_nodes + num_hyperedges
    adj_bipart, _ = add_self_loops(adj_bipart, num_nodes = total_nodes)
    # print(num_nodes + num_hyperedges ,H_sparse.shape[0]*2)
